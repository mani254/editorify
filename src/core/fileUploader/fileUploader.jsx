...
const FileUploader = ({ containerId, maxFiles = -1, maxFileSize = -1, validTypes = ["image/jpeg", "image/webp", "image/png", "image/gif"], onFilesChange = null, loadedFiles = null }) => {
	...
	const handleFileChange = useCallback(
		async (event) => {
			...
			for (const file of files) {
				if (maxFileSize > 0 && file.size / 1024 > maxFileSize) {
					alert(`File ${file.name} exceeds the maximum file size of ${maxFileSize} KB.`);
					continue;
				}
	...
	const handleDragOver = useCallback((index) => {
		...
		const draggedImage = newPreviewImages[dragIndex];
		newPreviewImages.splice(dragIndex, 1);
		newPreviewImages.splice(index, 0, draggedImage);

		const newImages = [...images];
		const draggedImg = newImages[dragIndex];
	...
	const handleUrl = useCallback(
			...
			try {
				...
				if (maxFileSize > 0) {
					if (image.size / 1024 > maxFileSize) {
						alert(`The image exceeds the maximum file size of ${maxFileSize} KB.`);
						return;
					}
				}

				if (image && validTypes.includes(image.type)) {
					let previewUrl = URL.createObjectURL(image);
		...
		[fetchImageFromUrl, maxFileSize, validTypes, setImages, setPreviewImages, setUrl, setShowUrlInput]
	);

	const handleUrls = useCallback(
		async (imgUrls) => {
			if (!imgUrls || !Array.isArray(imgUrls) || imgUrls.length === 0) return;
			...
			try {
				const fetchedImages = await Promise.all(
					imgUrls.map(async (imgUrl) => {
						try {
							...
							if (maxFileSize > 0 && image.size / 1024 > maxFileSize) {
								console.warn(`Skipping image from ${imgUrl}: exceeds max file size of ${maxFileSize} KB.`);
								return null;
							}

							if (image && validTypes.includes(image.type)) {
								return { image, previewUrl: URL.createObjectURL(image) };
							} else {
								console.warn(`Skipping image from ${imgUrl}: invalid file type.`);
								return null;
							}
						} catch (error) {
							console.warn(`Skipping image from ${imgUrl}: failed to load.`);
							return null;
						}
					})
				);
				...
				if (validImages.length > 0) {
					setImages((prev) => [...validImages.map((item) => item.image)]);
					setPreviewImages((prev) => [...validImages.map((item) => item.previewUrl)]);
				}
			} catch (err) {
				alert("Failed to process image URLs");
			}
		},
		[fetchImageFromUrl, maxFileSize, validTypes, setImages, setPreviewImages]
	);

	return (
		<div id={containerId} className="image-container">
			<div className="image-grid">
				{previewImages.length ? (
					<div className="image-items">
						{previewImages.map((img, index) => {
							return (
								<div
									className={`cover-image-wrapper ${isDragging && dragIndex === index ? "dragging" : ""}`}
									key={index}
									draggable={true}
				...
				{showUrlInput && (previewImages.length < maxFiles || maxFiles < 0) && (
					<div className="url-input">
						...
						<button type="button" onClick={() => handleUrl(url)}>
							Add Image
						</button>
					</div>
				)}
